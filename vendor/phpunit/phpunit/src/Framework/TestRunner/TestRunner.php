    ...
    public function run(TestCase $test): void
    {
        ...
        if (!$error && !$failure && !$incomplete && !$skipped && !$risky &&
            ...
            Facade::emitter()->testConsideredRisky(
                ...
                'This tests does not define a code coverage target but is expected to do so',
            );
        ...
        if ($test->doesNotPerformAssertions() &&
            ...
            Facade::emitter()->testConsideredRisky(
                $test->valueObjectForEvents(),
                sprintf(
                    'This tests is not expected to perform assertions but performed %d assertion%s',
    ...
    private function runTestWithTimeout(TestCase $test): bool
    {
        ...
        try {
            ...
            Facade::emitter()->testConsideredRisky(
                $test->valueObjectForEvents(),
                sprintf(
                    'This tests was aborted after %d second%s',
                    $_timeout,
                    $_timeout !== 1 ? 's' : '',
                ),
            );
