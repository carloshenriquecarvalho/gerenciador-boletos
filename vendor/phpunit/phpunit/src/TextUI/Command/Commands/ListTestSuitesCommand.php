...
final readonly class ListTestSuitesCommand implements Command
{
    ...
    public function execute(): Result
    {
        ...
        $buffer .= sprintf(
            'Available tests suite%s:' . PHP_EOL,
            count($suites) > 1 ? 's' : '',
        );
    ...
    private function warnAboutConflictingOptions(): string
    {
        ...
        if ($configuration->hasDefaultTestSuite()) {
            $buffer .= 'The defaultTestSuite (XML) and --list-suites (CLI) options cannot be combined, only the default tests suite is shown' . PHP_EOL;
        }
