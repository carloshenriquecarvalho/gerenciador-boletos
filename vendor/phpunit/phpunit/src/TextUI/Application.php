        ...
        try {
            ...
            if ($testDoxResult !== null &&
                ...
                try {
                    ...
                    EventFacade::emitter()->testRunnerTriggeredPhpunitWarning(
                        sprintf(
                            'Cannot log tests results in TestDox HTML format to "%s": %s',
            ...
            if ($testDoxResult !== null &&
                ...
                try {
                    ...
                    EventFacade::emitter()->testRunnerTriggeredPhpunitWarning(
                        sprintf(
                            'Cannot log tests results in TestDox plain text format to "%s": %s',
    ...
    private function registerLogfileWriters(Configuration $configuration): void
    {
        ...
        if ($configuration->hasLogfileJunit()) {
            try {
                ...
                EventFacade::emitter()->testRunnerTriggeredPhpunitWarning(
                    sprintf(
                        'Cannot log tests results in JUnit XML format to "%s": %s',
        ...
        if ($configuration->hasLogfileOtr()) {
            try {
                ...
                EventFacade::emitter()->testRunnerTriggeredPhpunitWarning(
                    sprintf(
                        'Cannot log tests results in Open Test Reporting XML format to "%s": %s',
                        $configuration->logfileOtr(),
                        $e->getMessage(),
                    ),
                );
        ...
        if ($configuration->hasLogfileTeamcity()) {
            try {
                ...
                EventFacade::emitter()->testRunnerTriggeredPhpunitWarning(
                    sprintf(
                        'Cannot log tests results in TeamCity format to "%s": %s',
                        $configuration->logfileTeamcity(),
                        $e->getMessage(),
                    ),
                );
