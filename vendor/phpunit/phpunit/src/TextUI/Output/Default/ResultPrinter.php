...
final class ResultPrinter
{
    ...
    private function printTestRunnerNotices(TestResult $result): void
    {
        ...
        $this->printListHeaderWithNumber(count($elements), 'PHPUnit tests runner notice');
    ...
    private function printTestRunnerWarnings(TestResult $result): void
    {
        ...
        $this->printListHeaderWithNumber(count($elements), 'PHPUnit tests runner warning');
        $this->printList($elements);
    }
    ...
    private function printTestRunnerDeprecations(TestResult $result): void
    {
        ...
        $this->printListHeaderWithNumber(count($elements), 'PHPUnit tests runner deprecation');
        $this->printList($elements);
    }
    ...
    private function printRiskyTests(TestResult $result): void
    {
        ...
        $this->printListHeaderWithNumber($elements['numberOfTestsWithIssues'], 'risky tests');
        $this->printList($elements['elements']);
    }
    ...
    private function printIncompleteTests(TestResult $result): void
    {
        ...
        $this->printListHeaderWithNumber(count($elements), 'incomplete tests');
        $this->printList($elements);
    }
    ...
    private function printSkippedTestSuites(TestResult $result): void
    {
        ...
        $this->printListHeaderWithNumber(count($elements), 'skipped tests suite');
        $this->printList($elements);
    }
    ...
    private function printSkippedTests(TestResult $result): void
    {
        ...
        $this->printListHeaderWithNumber(count($elements), 'skipped tests');
        $this->printList($elements);
    }
    ...
    private function printIssueList(string $type, array $issues, bool $stackTrace = false): void
    {
        ...
        $this->printListHeader(
            sprintf(
                '%d tests%s triggered %d %s%s:' . PHP_EOL . PHP_EOL,
                $numberOfTests,
                $numberOfTests !== 1 ? 's' : '',
        ...
        foreach ($issues as $issue) {
            ...
            if (!$issue->triggeredInTest()) {
                ...
                foreach ($triggeringTests as $triggeringTest) {
                    $body .= PHP_EOL . PHP_EOL . '* ' . $triggeringTest['tests']->id();
                    ...
                    if ($triggeringTest['tests']->isTestMethod()) {
                        $body .= PHP_EOL . '  ' . $triggeringTest['tests']->file() . ':' . $triggeringTest['tests']->line();
                    }
                }
            }
